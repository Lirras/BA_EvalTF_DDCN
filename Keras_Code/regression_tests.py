...
def direct_cascade_reg():
    ...
    hist = model.fit(a, b, batch_size=16, epochs=40, validation_data=(c, d), callbacks=[lr])
    ...
    model.add(keras.layers.Dense(units=256, activation='relu'))
    model.add(keras.layers.Dense(units=1, activation='linear'))
    hist_2 = model.fit(e, f, batch_size=16, epochs=40, validation_data=(g, h), callbacks=[lr])

    krl.unfreezing_all(model)
    lr, optim = krl.fine_tuning()
    # model.compile(optimizer=optim, loss=keras.losses.MeanSquaredError, metrics=['mae'])
    hist_3 = model.fit(e, f, batch_size=16, epochs=100, validation_data=(g, h), callbacks=[lr])
    ...
    plot.multiple_plots(plot.add_epoch_counter_to_df(pandas.concat([df_one, df_two, df_three])))
