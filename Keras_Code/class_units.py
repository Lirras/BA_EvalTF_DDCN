...
class Regression():
    ...
    def initialize(self, in_shape):
        ...
        self.network.compile(optimizer=keras.optimizers.RMSprop(learning_rate=1e-03), loss=keras.losses.MeanSquaredError, metrics=['mae'])

    def train(self, train, target, val_tr, val_lb):
        hist = self.network.fit(train, target, batch_size=16, epochs=10, validation_data=(val_tr, val_lb))
        return hist
...
class Classification():
    ...
    def train(self, train, target, val_tr, val_lb):
        hist = self.network.fit(train, target, batch_size=128, epochs=10, validation_data=(val_tr, val_lb))
        return hist
